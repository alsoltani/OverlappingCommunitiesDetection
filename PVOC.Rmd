...
permanence <- function(G,v){
  # Need to keep the former id that will be erased by the induced_graph operation.
  if( Reduce("&",neighbors(G,v)$community == V(G)[v]$community) ){
    c = transitivity(G,type = "local",vids = V(G)[v])
    if(is.na(c)){
      c = 0
    }
    return(c)
    ...
    D = degree(G, v= v) 
    c = transitivity(Gp,type = "local",vids = which(V(Gp)$id == v))
    if(is.na(c)){
      c = 0
    }
    return(I/(Emax*D) - (1- c))
...
VERTEX_REPLICATION  <- function(G,cluster,thre){
  ...
  for (v in V(G)){
    # Whebn i iterate on the vertices, i don't keep the parameter and therefore need to write V(G)[v]
